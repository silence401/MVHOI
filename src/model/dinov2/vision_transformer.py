...
class DinoVisionTransformer(nn.Module):
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], **kwargs):
        ...
        if kwargs.get("obj_pose", None) is not None:
            # import pdb; pdb.set_trace()
            motion_frames = kwargs.get("obj_pose")
            motion_frames = rearrange(motion_frames, "b s n c  -> b  1 s n c") 
            motion_tokens = self.prepare_tokens_with_masks(motion_frames)

            x = torch.cat([x, motion_tokens], dim=1)
            B = x.shape[0]
            S = x.shape[1]
